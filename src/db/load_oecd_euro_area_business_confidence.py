"""Load normalized OECD euro area business confidence data into PostgreSQL fact tables."""
...
def load_normalized_oecd_euro_area_business_confidence() -> pd.DataFrame:
    """Load normalized OECD euro area business confidence parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_euro_area_business_confidence_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD euro area business confidence rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD euro area business confidence load process."""
    normalized_df = load_normalized_oecd_euro_area_business_confidence()
    ...
    logger.info("OECD euro area business confidence load complete.")
