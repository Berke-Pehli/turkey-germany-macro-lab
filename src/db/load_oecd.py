"""Load normalized OECD data into PostgreSQL fact tables."""
...
def load_normalized_oecd_turkiye_cpi() -> pd.DataFrame:
    """Load normalized OECD Türkiye CPI parquet data from the final data layer."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_turkiye_cpi_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Türkiye CPI load process."""
    normalized_df = load_normalized_oecd_turkiye_cpi()
    ...
    logger.info("OECD Türkiye CPI load complete.")
