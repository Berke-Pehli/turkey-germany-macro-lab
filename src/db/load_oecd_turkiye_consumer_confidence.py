"""Load normalized OECD Türkiye consumer confidence data into PostgreSQL fact tables."""
...
def load_normalized_oecd_turkiye_consumer_confidence() -> pd.DataFrame:
    """Load normalized OECD Türkiye consumer confidence parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_turkiye_consumer_confidence_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD Türkiye consumer confidence rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Türkiye consumer confidence load process."""
    normalized_df = load_normalized_oecd_turkiye_consumer_confidence()
    ...
    logger.info("OECD Türkiye consumer confidence load complete.")
