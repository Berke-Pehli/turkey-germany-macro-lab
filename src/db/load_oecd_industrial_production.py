"""Load normalized OECD Türkiye industrial production data into PostgreSQL fact tables."""
...
def load_normalized_oecd_turkiye_industrial_production() -> pd.DataFrame:
    """Load normalized OECD Türkiye industrial production parquet data from the final data layer."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_turkiye_industrial_production_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD industrial production rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Türkiye industrial production load process."""
    normalized_df = load_normalized_oecd_turkiye_industrial_production()
    ...
    logger.info("OECD Türkiye industrial production load complete.")
