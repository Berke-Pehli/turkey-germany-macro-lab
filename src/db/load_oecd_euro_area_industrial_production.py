"""Load normalized OECD euro area industrial production data into PostgreSQL fact tables."""
...
def load_normalized_oecd_euro_area_industrial_production() -> pd.DataFrame:
    """Load normalized OECD euro area industrial production parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_euro_area_industrial_production_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD euro area industrial production rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD euro area industrial production load process."""
    normalized_df = load_normalized_oecd_euro_area_industrial_production()
    ...
    logger.info("OECD euro area industrial production load complete.")
