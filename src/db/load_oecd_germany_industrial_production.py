"""Load normalized OECD Germany industrial production data into PostgreSQL fact tables."""
...
def load_normalized_oecd_germany_industrial_production() -> pd.DataFrame:
    """Load normalized OECD Germany industrial production parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_germany_industrial_production_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD Germany industrial production rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Germany industrial production load process."""
    normalized_df = load_normalized_oecd_germany_industrial_production()
    ...
    logger.info("OECD Germany industrial production load complete.")
