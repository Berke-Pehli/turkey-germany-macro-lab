"""Load normalized OECD Türkiye unemployment data into PostgreSQL fact tables."""
...
def load_normalized_oecd_turkiye_unemployment() -> pd.DataFrame:
    """Load normalized OECD Türkiye unemployment parquet data from the final data layer."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_turkiye_unemployment_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD unemployment rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Türkiye unemployment load process."""
    normalized_df = load_normalized_oecd_turkiye_unemployment()
    ...
    logger.info("OECD Türkiye unemployment load complete.")
