"""Load normalized OECD Germany business confidence data into PostgreSQL fact tables."""
...
def load_normalized_oecd_germany_business_confidence() -> pd.DataFrame:
    """Load normalized OECD Germany business confidence parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_germany_business_confidence_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD Germany business confidence rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD Germany business confidence load process."""
    normalized_df = load_normalized_oecd_germany_business_confidence()
    ...
    logger.info("OECD Germany business confidence load complete.")
