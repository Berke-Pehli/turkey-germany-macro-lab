"""Load normalized OECD euro area unemployment data into PostgreSQL fact tables."""
...
def load_normalized_oecd_euro_area_unemployment() -> pd.DataFrame:
    """Load normalized OECD euro area unemployment parquet data."""
    input_path = FINAL_DATA_DIR / "oecd" / "oecd_euro_area_unemployment_normalized.parquet"
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized OECD euro area unemployment rows to fact table structure."""
...
def main() -> None:
    """Run the normalized OECD euro area unemployment load process."""
    normalized_df = load_normalized_oecd_euro_area_unemployment()
    ...
    logger.info("OECD euro area unemployment load complete.")
