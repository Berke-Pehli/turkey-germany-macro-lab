"""Load normalized CBRT FX data into PostgreSQL fact tables.

This module reads the normalized CBRT FX parquet output, maps business keys
...
def load_normalized_cbrt_fx() -> pd.DataFrame:
    """Load normalized CBRT FX parquet data from the final data layer.

    Returns:
        A pandas DataFrame containing normalized CBRT FX observations.
    """
    input_path = FINAL_DATA_DIR / "cbrt_fx" / "cbrt_fx_normalized.parquet"
...
def fetch_dimension_lookup(query: str) -> pd.DataFrame:
    ...
    Returns:
        A pandas DataFrame containing the lookup result.
    """
    engine = get_engine()
...
def prepare_fact_macro_observation_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Map normalized CBRT FX rows to fact table structure.

    Args:
        df: Normalized CBRT FX DataFrame with business key columns.
...
def validate_prepared_fact_rows(df: pd.DataFrame) -> None:
    ...
    Args:
        df: Prepared fact table DataFrame.

    Raises:
        ValueError: If required ID columns contain missing values.
    """
    ...
    if invalid_columns:
        raise ValueError(
            f"Missing dimension mappings detected: {invalid_columns}"
        )
...
def load_fact_macro_observation(df: pd.DataFrame) -> None:
    ...
    Args:
        df: Prepared fact table DataFrame ready for insertion.
    """
    engine = get_engine()
...
def main() -> None:
    """Run the normalized CBRT FX load process."""
    normalized_df = load_normalized_cbrt_fx()
    ...
    logger.info("CBRT FX load complete.")
