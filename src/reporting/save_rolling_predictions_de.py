"""Save rolling prediction results for Germany.

This script runs the same expanding-window one-step-ahead evaluation used
in the Germany rolling benchmark and saves the prediction results to a CSV
file for reporting and dashboard use.

Input:
    - data/final/model_inputs/model_input_de_v1.parquet
...
def load_prepared_model_data_de() -> pd.DataFrame:
    """Load the prepared Germany modeling dataset.

    Returns:
        A pandas DataFrame containing the cleaned Germany modeling data
...
def main() -> None:
    """Create and save rolling prediction results for Germany."""
    ...
    output_path = output_dir / "rolling_predictions_de.csv"
    results_df.to_csv(output_path, index=False)

    logger.info("Saved Germany rolling predictions: %s", output_path)
