"""Save rolling prediction results for the euro area.

This script runs the same expanding-window one-step-ahead evaluation used
in the euro area rolling benchmark and saves the prediction results to a
CSV file for reporting and dashboard use.

Input:
    - data/final/model_inputs/model_input_ea_v1.parquet
...
def load_prepared_model_data_ea() -> pd.DataFrame:
    """Load the prepared euro area modeling dataset.
...
def main() -> None:
    """Create and save rolling prediction results for the euro area."""
    ...
    output_path = output_dir / "rolling_predictions_ea.csv"
    results_df.to_csv(output_path, index=False)

    logger.info("Saved euro area rolling predictions: %s", output_path)
