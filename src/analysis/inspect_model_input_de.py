"""Inspect Germany model input parquet for first modeling setup.

This script loads the exported Germany model-input parquet file and checks
its structure, date coverage, and missingness before baseline modeling.

Current purpose:
    - verify that the Germany model-input export is ready for the first
...
def main() -> None:
    """Load and inspect the Germany model input dataset."""
    input_path = FINAL_DATA_DIR / "model_inputs" / "model_input_de_v1.parquet"
    df = pd.read_parquet(input_path)

    logger.info("Loaded Germany model input rows: %s", len(df))
