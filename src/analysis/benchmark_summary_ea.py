"""Summarize euro area baseline benchmark results.

This script compares simple benchmark models for one-month-ahead inflation
in the euro area. It reports both in-sample and out-of-sample results for
the naive baseline and the linear regression model.
...
def load_model_data_ea() -> pd.DataFrame:
    """Load and prepare the euro area modeling dataset.
...
def main() -> None:
    """Compute and report benchmark summary results for the euro area."""
    df = load_model_data_ea().copy()
