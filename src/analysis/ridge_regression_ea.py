"""Run a Ridge regression benchmark for euro area inflation.

This script evaluates a Ridge regression model for one-month-ahead
inflation in the euro area using a time-based train/test split. It is an
optional robustness check after the naive model and plain linear regression.
...
def load_prepared_model_data_ea() -> pd.DataFrame:
    ...
    Returns:
        A pandas DataFrame containing the cleaned euro area baseline
...
def main() -> None:
    """Fit and evaluate a Ridge regression benchmark for the euro area."""
