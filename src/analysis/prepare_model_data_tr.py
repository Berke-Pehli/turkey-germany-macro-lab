"""Prepare Türkiye model data for the first inflation baseline.

This script loads the exported Türkiye model-input parquet file, keeps a
...
def load_model_input_tr() -> pd.DataFrame:
    """Load the exported Türkiye model-input dataset.

    Returns:
        A pandas DataFrame containing the Türkiye modeling view exported
        from PostgreSQL.
    """
...
def prepare_baseline_model_data_tr(df: pd.DataFrame) -> pd.DataFrame:
    """Prepare a clean baseline modeling dataset for Türkiye.

    Args:
        df: Raw Türkiye model-input DataFrame.
...
def main() -> None:
    """Load and prepare the first Türkiye baseline modeling dataset."""
    raw_df = load_model_input_tr()
    model_df = prepare_baseline_model_data_tr(raw_df)

    logger.info("Raw Türkiye model-input rows: %s", len(raw_df))
